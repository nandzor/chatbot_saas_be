/**
 * Edit Bot Personality Dialog
 * Dialog untuk mengedit bot personality dengan assignment features
 */
...
const EditBotPersonalityDialog = ({ open, onOpenChange, personality, onPersonalityUpdated }) => {
  ...
  useEffect(() => {
    if (personality && open) {
      setFormData({
        ...
        // Assignment fields
        n8n_workflow_id: personality.n8n_workflow_id || null,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await botPersonalityService.update(personality.id, formData);

      if (response.success) {
        toast.success('Bot personality updated successfully');
        onPersonalityUpdated(response.data);
        onOpenChange(false);
      } else {
        throw new Error(response.message || 'Failed to update bot personality');
      }
    } catch (error) {
      handleError(error, { context: 'Update Bot Personality' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Edit Bot Personality: {personality.name}
          </DialogTitle>
          <DialogDescription>
            Update bot personality configuration and integrations.
          </DialogDescription>
        </DialogHeader>
...
export default EditBotPersonalityDialog;

...
const CreatePermissionDialog = ({ isOpen, onClose, onSubmit, loading = false }) => {
  ...
  useEffect(() => {
    if (formData.resource && formData.action && !formData.name) {
      const generatedCode = `${formData.resource}.${formData.action}`;
      const generatedName = `${formData.action.charAt(0).toUpperCase() + formData.action.slice(1)} ${formData.resource.charAt(0).toUpperCase() + formData.resource.slice(1)}`;
  ...
  }, [formData.resource, formData.action, formData.name]);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <Alert>
            ...
            <AlertDescription>
              ...
              System permissions are typically created by the system and should be used carefully.
            </AlertDescription>
          </Alert>

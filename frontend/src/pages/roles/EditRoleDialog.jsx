...
const EditRoleDialog = ({ isOpen, onClose, role, onSubmit, loading = false }) => {
  ...
  // Load role data from API if only ID is provided
  const loadRoleData = useCallback(async () => {
    ...
    try {
      ...
      // If role is just an ID, fetch the full data
      let roleData = role;
      if (typeof role === 'string' || (role && role.id && !role.name)) {
        const response = await roleManagementService.getRole(role.id || role);
        if (response.success) {
      ...
      setFormData({
        ...
        is_system_role: Boolean(roleData.is_system_role),
        is_default: Boolean(roleData.is_default),
        is_active: roleData.status === 'active',
        max_users: roleData.max_users || null,
        color: roleData.color || '#3B82F6',
        icon: roleData.icon || 'shield',
        badge_text: roleData.badge_text || '',
        inherits_permissions: Boolean(roleData.inherits_permissions),
    ...
    try {
      ...
      const roleData = {
        ...
        metadata: {
          ...
          system_role: formData.is_system_role,
          dangerous_role: formData.level >= 90 // Mark high-level roles as dangerous
        }
      };

      // Get role ID
      const roleId = typeof role === 'string' ? role : (role?.id || role);
      ...
      if (response.success) {
        ...
        let updatedRoleData = response.data;
        ...
        toast.success(`Role "${updatedRoleData.name || response.data}" has been updated successfully`);

        // Call parent onSubmit if provided
        if (onSubmit) {
          await onSubmit(updatedRoleData);
        }
  ...
  }, [formData, onSubmit, validateForm, onClose, role]);
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
          <div className="flex items-center justify-center h-64">
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg text-gray-600">Loading role data...</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Modify role settings and permissions</p>
            </div>
          </div>
          <Button
            variant="ghost"
          ...
          <div className="p-6 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Modify the role's identity and basic properties
                </CardDescription>
              </CardHeader>

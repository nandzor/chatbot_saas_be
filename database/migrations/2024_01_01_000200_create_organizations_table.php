...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('organizations', function (Blueprint $table) {
            ...
            // Usage Tracking
            $table->json('current_usage')->default(json_encode([
                'messages' => 0,
                'ai_requests' => 0,
                'api_calls' => 0,
                'storage_mb' => 0,
            ...
            // UI/UX Configuration
            $table->json('theme_config')->default(json_encode(['primaryColor' => '#3B82F6', 'secondaryColor' => '#10B981', 'darkMode' => false]));
            $table->json('branding_config')->default('{}');
            $table->json('feature_flags')->default('{}');
            $table->json('ui_preferences')->default('{}');

            // Business Configuration
            $table->json('business_hours')->default(json_encode(['timezone' => 'Asia/Jakarta', 'days' => []]));
            $table->json('contact_info')->default('{}');
            $table->json('social_media')->default('{}');

            // Security Settings
            $table->json('security_settings')->default(json_encode([

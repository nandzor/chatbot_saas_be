...
class KnowledgeBaseSeeder extends Seeder
{
    ...
    private function createTags(Organization $organization): array
    {
        ...
        foreach ($tagNames as $tagName) {
            $tag = KnowledgeBaseTag::firstOrCreate([
                ...
                'icon' => $this->getTagIcon($tagName),
                'tag_type' => $this->getTagType($tagName),
                'usage_count' => 0,
                'item_count' => 0,
                'is_system_tag' => in_array($tagName, ['beginner', 'advanced', 'tutorial', 'faq']),
                'is_auto_suggested' => $this->faker->boolean(80),
                'auto_apply_rules' => [],
                'status' => 'active',
            ]);
    ...
    private function createArticlesForCategory(Organization $organization, KnowledgeBaseCategory $category, $users, array $tags): void
    {
        ...
        for ($i = 0; $i < $articleCount; $i++) {
            ...
            $item = KnowledgeBaseItem::create([
                ...
                'title' => $title,
                'slug' => \Illuminate\Support\Str::slug($title) . '-' . $i . '-' . \Illuminate\Support\Str::random(8),
                ...
                'ai_last_processed_at' => $this->faker->optional(0.3)->dateTimeBetween('-6 months', 'now'),
                'version' => 1,
                'is_latest_version' => true,
                'quality_score' => $this->faker->randomFloat(2, 0.6, 1.0),
                'effectiveness_score' => $this->faker->randomFloat(2, 0.5, 1.0),
                'last_effectiveness_update' => $this->faker->optional(0.5)->dateTimeBetween('-3 months', 'now'),
                'metadata' => [],
                'configuration' => [],
                'status' => 'draft',
            ]);
    ...
    private function createQaItems(Organization $organization, array $categories, $users, array $tags): void
    {
        ...
        for ($i = 0; $i < $qaCount; $i++) {
            ...
            $item = KnowledgeBaseItem::create([
                ...
                'author_id' => $users->random()->id,
                'title' => $question,
                'slug' => \Illuminate\Support\Str::slug($question) . '-' . $i . '-' . \Illuminate\Support\Str::random(8),
                'description' => $this->faker->sentence(),
                'content_type' => 'faq',
                'content' => $answer,
                ...
                'language' => 'indonesia',
                'difficulty_level' => $this->faker->randomElement(['basic', 'intermediate']),
        ...
        foreach ($featuredCategories as $categorySlug) {
            if (isset($categories[$categorySlug])) {
                ...
                $item = KnowledgeBaseItem::create([
                    ...
                    'slug' => "panduan-lengkap-{$category->slug}-" . \Illuminate\Support\Str::random(8),
                ...
                'comment_count' => $this->faker->numberBetween(50, 200),
                ...
                'ai_last_processed_at' => $this->faker->dateTimeBetween('-3 months', 'now'),
                'version' => 1,
                'is_latest_version' => true,
                'quality_score' => 0.95,
                'effectiveness_score' => 0.90,
                'last_effectiveness_update' => $this->faker->dateTimeBetween('-1 month', 'now'),
                'metadata' => [],
                'configuration' => [],
                'status' => 'published',
                ]);
        ...
        foreach ($knowledgeItems as $item) {
            ...
            for ($i = 0; $i < $qaCount; $i++) {
                \App\Models\KnowledgeQaItem::create([
                    ...
                    'question' => $this->generateQuestion($item->category) . ' #' . ($i + 1),
                    'answer' => $this->generateAnswer($item->category),
                    'question_variations' => [],
                    'answer_variations' => [],
                    'context' => $this->faker->optional(0.7)->sentence(),
                    'intent' => $this->faker->randomElement(['information', 'troubleshooting', 'how-to', 'explanation']),
                    'confidence_level' => $this->faker->randomElement(['low', 'medium', 'high']),
                    'keywords' => $this->generateKeywords($this->generateQuestion($item->category), $this->generateAnswer($item->category)),
                    'search_keywords' => [],
                    'trigger_phrases' => [],
                    'conditions' => [],
                    'response_rules' => [],
                    'usage_count' => $this->faker->numberBetween(0, 1000),
                    'success_rate' => $this->faker->randomFloat(2, 0.5, 1.0),
                    'user_satisfaction' => $this->faker->randomFloat(2, 0.6, 1.0),
                    'last_used_at' => $this->faker->optional(0.6)->dateTimeBetween('-3 months', 'now'),
                    'ai_confidence' => $this->faker->randomFloat(2, 0.7, 1.0),
                    'ai_embeddings' => [],
                    'ai_last_trained_at' => $this->faker->optional(0.4)->dateTimeBetween('-6 months', 'now'),
                    'order_index' => $i + 1,
                    'is_primary' => $i === 0,
                    'is_active' => true,
                    'metadata' => [
    ...
    private function getTagIcon(string $tagName): ?string
    {
        $tagIcons = [
            'beginner' => 'play',
            'advanced' => 'star',
    ...
    private function getTagType(string $tagName): string
    {
        ...
        return $tagTypes[$tagName] ?? 'general';
    }

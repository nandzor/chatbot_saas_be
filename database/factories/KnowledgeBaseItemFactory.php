...
class KnowledgeBaseItemFactory extends Factory
{
    ...
    private function assessContentFreshness(string $status): array
    {
        ...
        return [
            ...
            'next_review_due' => $this->faker->dateTimeBetween('now', '+6 months')->format('c')
        ];
    }
    ...
    private function generateReviewSchedule(string $contentType, string $status): array
    {
        ...
        return [
            'frequency' => $frequency,
            'last_reviewed' => $lastReview?->format('c'),
            'next_review_due' => $lastReview ? $lastReview->modify('+' . match($frequency) {
                'quarterly' => '3 months',

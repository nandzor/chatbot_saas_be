...
class SendPaymentSuccessEmail implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            if (!$organization) {
                Log::channel('payment')->warning('Organization not found for payment success email', [
            ...
            if (!$this->shouldSendEmail($organization)) {
                Log::channel('payment')->info('Payment success email skipped - notifications disabled', [
                    'payment_id' => $this->payment->id,
            ...
            Mail::to($organization->email)
                ->send(new \App\Mail\PaymentSuccessMail($this->payment, $organization));

            Log::channel('payment')->info('Payment success email sent', [
                'payment_id' => $this->payment->id,
                'organization_id' => $organization->id,
            ...
            Log::channel('payment')->error('Failed to send payment success email', [
                'payment_id' => $this->payment->id,
                'organization_id' => $this->payment->organization_id,
                'error' => $e->getMessage(),
    ...
    {
        Log::channel('payment')->error('Payment success email job failed', [
            'payment_id' => $this->payment->id,

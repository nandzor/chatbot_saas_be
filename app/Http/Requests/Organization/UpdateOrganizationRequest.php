...
class UpdateOrganizationRequest extends BaseRequest
{
    ...
    {
        return $this->user()->hasPermission('organizations.update');
    }
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->has('api_enabled')) {
            $this->merge(['api_enabled' => $this->boolean('api_enabled')]);
        }
        ...
        $arrayFields = [
            ...
            'security_settings', 'settings', 'metadata'
        ];

        foreach ($arrayFields as $field) {
            if ($this->has($field)) {
                $this->merge([$field => $this->get($field, [])]);
            }
        }
    }
}
